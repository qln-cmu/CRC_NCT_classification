        ...
        for inputs, labels in val_loader:
            ...
            for i in range(len(labels)):
                if labels[i] in cancer_set:
                    break
            cancer_labels = labels[i:]
            cancer_inputs = inputs[i:]
            cancer_inputs = cancer_inputs.to(device)
            cancer_labels = cancer_labels.to(device)
            
            cancer_outputs = model(cancer_inputs)
            cancer_loss = criterion(cancer_outputs, cancer_labels)
            
            cancer_val_loss += cancer_loss.item() * cancer_inputs.size(0)
            _, preds = torch.max(cancer_outputs, 1)
            cancer_val_corrects += torch.sum(preds == cancer_labels.data)
            cancer_val_total += cancer_inputs.size(0)
            
    
    val_epoch_loss = val_loss / len(val_dataset)
    val_epoch_acc = val_corrects.double() / len(val_dataset)
    cancer_val_epoch_loss = cancer_val_loss / cancer_val_total
    cancer_val_epoch_acc = cancer_val_corrects / cancer_val_total

    # Print stats and save to CSV
    print(f'Epoch {epoch+1}/{num_epochs} - Train Loss: {epoch_loss:.4f} Train Acc: {epoch_acc:.4f} - '
          f'Val Loss: {val_epoch_loss:.4f} Val Acc: {val_epoch_acc:.4f} - '
          f'Cancer Val Loss: {cancer_val_epoch_loss:.4f} Cancer Val Acc: {cancer_val_epoch_acc:.4f} - LR: {current_lr}')
    with open(log_path, 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([epoch+1, epoch_acc.item(), epoch_loss, val_epoch_acc.item(), val_epoch_loss, cancer_val_epoch_acc.item(), cancer_val_epoch_loss, current_lr])
